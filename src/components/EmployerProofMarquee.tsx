      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mb-4 sm:mb-5 relative z-0">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2.5 text-center sm:text-left">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] sm:text-xs font-mono uppercase tracking-widest text-zinc-400 light:text-zinc-600 font-semibold flex items-center gap-1.5">
              <span>Companies Where JobFlux AI Submits Your Applications</span>
            </span>
          </div>

          <div className="flex items-center gap-3 text-[11px] font-mono text-zinc-500 light:text-zinc-600">
            ...
            <span className="text-zinc-400 light:text-zinc-600 hidden sm:inline">Harvard ATS Optimised</span>
          </div>
        </div>
      </div>
      ...
      <div className="relative w-full max-w-full overflow-hidden touch-pan-y">
        <div className="animate-marquee-scroll flex gap-4 sm:gap-6 py-1">
          {[...TOP_EMPLOYERS, ...TOP_EMPLOYERS].map((emp, idx) => (
            <div
              key={`${emp.name}-${idx}`}
              className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-zinc-950/70 light:bg-white hover:bg-zinc-900/80 light:hover:bg-zinc-100 border border-zinc-800/80 light:border-zinc-200 hover:border-cyan-500/40 transition-all duration-300 group shrink-0 cursor-default shadow-md hover:shadow-cyan-500/10 pointer-events-auto"
            >
              <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${emp.logoBg} flex items-center justify-center text-white light:text-zinc-900 font-extrabold text-xs shadow-inner shrink-0 group-hover:scale-105 transition-transform`}>
                {emp.name.slice(0, 1)}
              </div>
              <div className="text-left">
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-bold text-white light:text-zinc-900 group-hover:text-cyan-300 transition-colors">
                    {emp.name}
                  </span>
                  <span className="text-[9px] font-mono px-1.5 py-0.2 rounded bg-cyan-950/60 light:bg-cyan-50 text-cyan-300 light:text-cyan-700 border border-cyan-800/50 light:border-cyan-300">
                    {emp.salary}
                  </span>
                </div>
                <div className="text-[10px] text-zinc-400 light:text-zinc-600 font-medium truncate max-w-[160px]">
                  {emp.targeting}
                </div>
              </div>
            </div>
