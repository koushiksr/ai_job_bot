...
export default function NeuralAtsDiagnosticCard({
  ...
  return (
    <div className="p-5 rounded-2xl bg-[#09090b] border border-zinc-800 space-y-4 relative overflow-hidden card-featured-glow text-zinc-100">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isProfessional ? (
                <span className="text-[9px] font-mono px-2 py-0.5 rounded bg-zinc-900 border border-zinc-800 text-zinc-300 font-semibold flex items-center gap-1">
                  <Sparkles className="w-2.5 h-2.5 text-zinc-400" /> NEURAL ACTIVE
                </span>
              ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-1">
        ...
        <div
          ...
          className={`p-3.5 rounded-xl bg-zinc-950 border transition-all space-y-2 ${
            isProfessional
              ? 'border-zinc-800/90'
              : 'border-zinc-800 hover:border-zinc-700 cursor-pointer group'
          }`}
        >
          <div className="flex items-center justify-between text-xs">
            <span className="text-zinc-400 flex items-center gap-1.5 font-medium">
              <Target className="w-3.5 h-3.5 text-zinc-400" /> Recruiter Keyword Alignment
            </span>
            {isProfessional ? (
              <span className="font-mono text-zinc-200 font-semibold text-[11px]">OPTIMIZED</span>
            ) : (
      ...
      {!isProfessional && (
        <div className="p-3.5 rounded-xl bg-zinc-950 border border-zinc-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
