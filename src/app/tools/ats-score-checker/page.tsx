...
export default function AtsScoreCheckerPage() {
  ...
  return (
    <div className="min-h-screen bg-[#000000] light:bg-white text-zinc-100 light:text-zinc-900 flex flex-col font-sans selection:bg-zinc-800 selection:text-white relative">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-5 w-full flex items-center gap-1.5 text-xs text-zinc-400 light:text-zinc-600">
        <Link href="/tools" className="hover:text-white light:hover:text-zinc-900 transition-colors">
          Free Tools
        </Link>
        <ChevronRight className="w-3.5 h-3.5 text-zinc-600" />
        <span className="text-zinc-200 light:text-zinc-800 font-medium">ATS Resume & Keyword Gap Checker</span>
      </div>
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 sm:px-6 py-8 relative z-10 w-full">
        <div className="text-center max-w-2xl mx-auto mb-10 space-y-3">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider bg-zinc-950 light:bg-white border border-zinc-800 light:border-zinc-200 text-zinc-300 light:text-zinc-700">
            <FileCheck2 className="w-3.5 h-3.5 text-cyan-400 light:text-cyan-600" /> Harvard ATS Parser Diagnostics
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-[#09090b] border border-zinc-800 light:border-zinc-200 rounded-2xl p-5 shadow-xl">
            ...
            <textarea
              ...
              className="w-full bg-black light:bg-white border border-zinc-800 light:border-zinc-200 rounded-xl p-3 text-xs text-zinc-300 light:text-zinc-700 font-mono focus:outline-none focus:border-zinc-600 transition-colors"
            />
          </div>
          ...
          <div className="bg-[#09090b] border border-zinc-800 light:border-zinc-200 rounded-2xl p-5 shadow-xl">
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                className="text-[11px] text-amber-400 light:text-amber-600 hover:text-amber-300 hover:underline cursor-pointer"
              >
                Reset Sample
              </button>
            </div>
        ...
        <div className="text-center mb-10">
          <button
            ...
            disabled={analyzing}
            className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white light:text-zinc-900 font-semibold text-xs sm:text-sm px-8 py-3 rounded-xl shadow-lg shadow-cyan-500/20 transition-all cursor-pointer"
          >
            {analyzing ? (
              <>
          ...
          <div className="bg-[#09090b] border border-zinc-800 light:border-zinc-200 rounded-3xl p-6 md:p-8 shadow-2xl mb-12">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
              ...
              <div className="md:col-span-4 text-center md:border-r md:border-zinc-800 md:pr-6">
                ...
                <p className="text-xs text-zinc-400 light:text-zinc-600 mt-2">
                  {score >= 80 ? (
                    <span className="text-emerald-400 light:text-emerald-600 font-medium">Excellent match! Ready for submission.</span>
                  ) : score >= 60 ? (
                    <span className="text-amber-400 light:text-amber-600 font-medium">Moderate match. Add missing keywords below.</span>
                  ) : (
                    <span className="text-rose-400 light:text-rose-600 font-medium">High risk of automatic rejection by ATS.</span>
                  )}
                </p>
              </div>
              ...
              <div className="md:col-span-8 space-y-5">
                <div>
                  <h3 className="text-xs font-bold text-zinc-200 light:text-zinc-800 flex items-center gap-1.5 mb-2.5">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 light:text-emerald-600" /> Matched Keywords Found in Resume
                  </h3>
                  <div className="flex flex-wrap gap-1.5">
                    {matchedKeywords.length > 0 ? (
                      matchedKeywords.map((kw, i) => (
                        <span key={i} className="text-[11px] bg-emerald-500/10 text-emerald-400 light:text-emerald-600 border border-emerald-500/20 px-2.5 py-1 rounded-lg font-medium">
                          ✓ {kw}
                        </span>
                      ))
                    ) : (
                      <span className="text-xs text-zinc-500 light:text-zinc-600">No direct keywords matched yet.</span>
                    )}
                  </div>
                </div>
                ...
                <div>
                  <h3 className="text-xs font-bold text-zinc-200 light:text-zinc-800 flex items-center gap-1.5 mb-2.5">
                    <AlertTriangle className="w-4 h-4 text-rose-400 light:text-rose-600" /> Missing High-Priority Keywords from JD
                  </h3>
                  <div className="flex flex-wrap gap-1.5">
                    {missingKeywords.length > 0 ? (
                      missingKeywords.map((kw, i) => (
                        <span key={i} className="text-[11px] bg-rose-500/10 text-rose-400 light:text-rose-600 border border-rose-500/20 px-2.5 py-1 rounded-lg font-medium">
                          + {kw}
                        </span>
                      ))
                    ) : (
                      <span className="text-xs text-emerald-400 light:text-emerald-600">All core technical keywords matched!</span>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="mt-8 pt-6 border-t border-zinc-800 light:border-zinc-200 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <Link
                href="/resume-builder"
                className="inline-flex items-center gap-2 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white light:text-zinc-900 font-semibold text-xs px-5 py-2.5 rounded-xl shadow-lg shadow-cyan-500/20 transition-all"
              >
