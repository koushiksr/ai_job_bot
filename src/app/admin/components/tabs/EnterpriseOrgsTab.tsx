    ...
    <div className="space-y-6">
      ...
      <div className="p-5 rounded-2xl bg-gradient-to-r from-indigo-950/40 via-zinc-900 to-black border border-indigo-500/20 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Link href="/enterprise-admin" target="_blank" className="px-3 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold flex items-center gap-1.5 shadow-lg transition-all cursor-pointer">
      ...
      {statusMessage && (
        <div className={`p-3.5 rounded-xl border flex items-center justify-between text-xs ${
          ...
          <button type="button" onClick={() => setStatusMessage(null)} className="text-zinc-500 hover:text-white cursor-pointer ml-4">✕</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Total Orgs', value: enterpriseOrgs.length, sub: 'Multi-tenant cohorts', icon: <Building2 className="w-3.5 h-3.5 text-indigo-400" />, color: 'text-white' },
          { label: 'Enterprise Members', value: totalMembers, sub: 'Across all orgs', icon: <Users className="w-3.5 h-3.5 text-cyan-400" />, color: 'text-cyan-300' },
          { label: "Today's Runs", value: todayApplied, sub: '55 max/user/day', icon: <Zap className="w-3.5 h-3.5 text-amber-400" />, color: 'text-amber-300' },
          { label: 'Total Applied', value: totalApplied, sub: 'All-time submissions', icon: <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />, color: 'text-emerald-300' },
        ].map(card => (
          <div key={card.label} className="p-4 rounded-xl bg-[#09090b] border border-zinc-800 flex flex-col justify-between">
            ...
            <div className="text-[10px] text-zinc-500 mt-1">{card.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl bg-[#09090b] border border-zinc-800 overflow-hidden shadow-xl">
        <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold text-white uppercase tracking-wider">Provisioned Organizations ({enterpriseOrgs.length})</h3>
          </div>
          <span className="text-[11px] text-zinc-500">Expand to manage members · Click actions to control access</span>
        </div>
        ...
        {loadingOrgs ? (
          ...
          <div className="py-12 text-center text-zinc-500">
            ...
            <p className="font-semibold text-zinc-400">No organizations yet.</p>
                ...
                <div key={org.org_id} className={isDisabled ? 'bg-rose-950/10' : ''}>
                  ...
                  <div className="flex flex-wrap items-center gap-3 py-4 px-4 hover:bg-zinc-900/30 transition-colors">
                    ...
                    <div className="flex items-center gap-2.5 min-w-[180px] flex-1">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border ${isDisabled ? 'bg-rose-500/10 border-rose-500/20 text-rose-400' : 'bg-indigo-500/10 border-indigo-500/20 text-indigo-400'}`}>
                        <Building2 className="w-4 h-4" />
                      </div>
                      <div>
                        <div className="font-bold text-white text-xs flex items-center gap-1.5 flex-wrap">
                          ...
                          {org.org_id === 'org_technohmsit' && (
                            <span className="px-1.5 rounded text-[9px] font-mono uppercase bg-amber-500/20 text-amber-300 border border-amber-500/30">Primary</span>
                          )}
                          {isDisabled && (
                            <span className="px-1.5 rounded text-[9px] font-mono uppercase bg-rose-500/20 text-rose-300 border border-rose-500/30">DISABLED</span>
                          )}
                        </div>
                        <div className="text-[10px] font-mono text-zinc-500 mt-0.5">{org.org_id}</div>
                      </div>
                    </div>

                    {/* Admin */}
                    <div className="min-w-[150px]">
                      <div className="font-semibold text-zinc-200 text-xs">{org.admin_name || org.admin_email.split('@')[0]}</div>
                    ...
                    <div className="flex items-center gap-4 text-xs font-mono text-zinc-300">
                      <div className="text-center">
                        <div className="font-bold text-cyan-300">{org.member_count}</div>
                        <div className="text-[10px] text-zinc-500">members</div>
                      </div>
                      <div className="text-center">
                        <span className="text-emerald-400 font-bold">{org.today_applied}</span>
                        <span className="text-zinc-600 mx-1">/</span>
                        <span className="text-zinc-300">{org.total_applied}</span>
                        <div className="text-[10px] text-zinc-500">today/total</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 ml-auto flex-wrap justify-end">
                      ...
                      <button type="button" onClick={() => toggleExpand(org.org_id)}
                        className="px-2.5 py-1.5 rounded-lg bg-zinc-900 hover:bg-zinc-800 border border-zinc-700 text-zinc-300 hover:text-white text-[11px] font-semibold flex items-center gap-1 transition-colors cursor-pointer"
                      ...
                      <button type="button"
                        ...
                        className={`px-2.5 py-1.5 rounded-lg border text-[11px] font-semibold flex items-center gap-1 transition-colors cursor-pointer ${
                          isDisabled
                            ? 'bg-emerald-950/30 hover:bg-emerald-950/50 border-emerald-800/50 text-emerald-300 hover:text-emerald-200'
                            : 'bg-zinc-900 hover:bg-rose-950/30 border-zinc-700 hover:border-rose-500/40 text-zinc-300 hover:text-rose-300'
                        }`}
                  ...
                  {isDisabled && (
                    <div className="mx-4 mb-3 p-3 rounded-xl bg-rose-950/40 border border-rose-500/30 flex items-center gap-2 text-xs text-rose-300">
                  ...
                  {isExpanded && (
                    <div className="mx-4 mb-4 rounded-xl bg-zinc-950 border border-zinc-800 overflow-hidden">
                      <div className="p-3 border-b border-zinc-800 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Users className="w-3.5 h-3.5 text-cyan-400" />
                          <span className="text-xs font-bold text-white">Members of {org.name}</span>
                        ...
                        <div className="flex items-center gap-2">
                          <button type="button" onClick={() => loadOrgMembers(org.org_id)} disabled={loadingThis}
                            className="p-1.5 rounded-lg bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-400 hover:text-white cursor-pointer transition-colors">
                            ...
                            return (
                              <div key={member.user_id} className="flex items-center gap-3 px-4 py-3 hover:bg-zinc-900/30 transition-colors">
                                ...
                                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${
                                  member.enterprise_role === 'admin' ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-indigo-500/10 text-indigo-300 border border-indigo-500/20'
                                }`}>
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-1.5 text-xs font-semibold text-white truncate">
                                    ...
                                    {member.is_org_admin_only && (
                                      <span className="text-[9px] px-1 rounded bg-zinc-800 text-zinc-400 font-mono">admin only</span>
                                    )}
                                  </div>
                                  <div className="text-[10px] text-zinc-500 truncate">{member.email}</div>
                                </div>
                                ...
                                <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold uppercase font-mono shrink-0 ${
                                  member.enterprise_status === 'disabled' ? 'bg-rose-500/20 text-rose-300 border border-rose-500/30' : 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/30'
                                }`}>
                                  ...
                                  <button type="button"
                                    ...
                                    className={`p-1.5 rounded-lg border cursor-pointer transition-colors ${
                                      member.enabled_for_daily_run
                                        ? 'bg-emerald-950/30 border-emerald-800/50 text-emerald-400 hover:bg-rose-950/30 hover:border-rose-500/40 hover:text-rose-400'
                                        : 'bg-zinc-900 border-zinc-700 text-zinc-500 hover:bg-emerald-950/30 hover:border-emerald-800/50 hover:text-emerald-400'
                                    }`}
                                  >
                                ...
                                <button type="button"
                                  ...
                                  className="p-1.5 rounded-lg bg-zinc-900 hover:bg-rose-950/30 border border-zinc-800 hover:border-rose-500/40 text-zinc-500 hover:text-rose-400 cursor-pointer transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-zinc-950 border border-zinc-800 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-zinc-800">
              ...
              <button type="button" onClick={() => setShowCreateModal(false)} className="text-zinc-500 hover:text-white cursor-pointer p-1">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateOrg} className="space-y-3.5 text-xs">
              ...
              <div>
                ...
                <input type="email" required placeholder="admin@example.com" value={newAdminEmail}
                  ...
                  className="w-full px-3 py-2 rounded-xl bg-zinc-900 border border-zinc-800 text-white placeholder:text-zinc-600 focus:outline-none focus:border-indigo-500" />
                <p className="text-[10px] text-zinc-500 mt-1">Must be a registered JobFlux user. They get access to /enterprise-admin.</p>
              </div>
              <div>
                <label className="block text-zinc-300 font-semibold mb-1">Admin Display Name</label>
              ...
              <div className="grid grid-cols-2 gap-3 pt-1">
                <div>
                  <label className="block text-zinc-400 text-[11px] mb-1">Daily Limit / Member</label>
                  ...
                  <span className="text-[9px] text-zinc-500">Max 55</span>
                </div>
                <div>
                  <label className="block text-zinc-400 text-[11px] mb-1">Weekly On-Demand</label>
                  <input type="number" min={1} value={newOnDemandQuota} onChange={e => setNewOnDemandQuota(Number(e.target.value))}
                    className="w-full px-3 py-2 rounded-xl bg-zinc-900 border border-zinc-800 text-white font-mono focus:outline-none focus:border-indigo-500" />
                  <span className="text-[9px] text-zinc-500">Default 10/wk</span>
                </div>
              </div>
              <div className="pt-3 flex items-center justify-end gap-2 border-t border-zinc-800">
                <button type="button" onClick={() => setShowCreateModal(false)}
                  className="px-3.5 py-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-300 font-semibold cursor-pointer">Cancel</button>
                <button type="submit" disabled={creating}
                  className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold flex items-center gap-1.5 shadow-lg cursor-pointer disabled:opacity-60">
      ...
      {assignAdminModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-zinc-950 border border-zinc-800 rounded-2xl max-w-sm w-full p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-zinc-800">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-bold text-white">Assign Enterprise Admin</h3>
                  <p className="text-[11px] text-zinc-400">{assignAdminModal.name}</p>
                </div>
              </div>
              <button type="button" onClick={() => setAssignAdminModal(null)} className="text-zinc-500 hover:text-white cursor-pointer">
                <X className="w-4 h-4" />
              </button>
            </div>

            <p className="text-xs text-zinc-400 leading-relaxed">
              Enter the email of an <strong className="text-white">existing</strong> registered JobFlux user. They will be promoted to <strong className="text-amber-300">Enterprise Admin</strong> for this org and can access the Enterprise Admin Portal.
            </p>
            ...
            <form onSubmit={handleAssignAdmin} className="space-y-3">
              <div className="relative">
                ...
                <input type="email" required value={assignAdminEmail}
                  ...
                  className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-zinc-900 border border-zinc-800 text-white text-sm placeholder-zinc-600 focus:outline-none focus:border-amber-500" />
              </div>
              <div className="flex items-center gap-2 pt-2">
                <button type="button" onClick={() => setAssignAdminModal(null)}
                  className="flex-1 py-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-300 font-semibold text-xs cursor-pointer">Cancel</button>
                <button type="submit" disabled={assigningAdmin}
                  className="flex-1 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white font-semibold text-xs flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-60">
