...
import { getDailyAppLimit, getCapUpgradeHint, getWeeklyOnDemandLimit, ORG_PLANS } from '@/config/plans'
...
export default function UserDashboard() {
  ...
  // Deep-link anchor handling for #plans
  useEffect(() => {
    const handleHashNavigation = () => {
      if (typeof window === 'undefined') return
      if (window.location.hash === '#plans') {
        const storedPlan = localStorage.getItem('user_plan') || userPlan
        const storedRole = localStorage.getItem('user_role')
        const storedEnterpriseRole = localStorage.getItem('enterprise_role') || enterpriseRole
        ...
        if (isOrg) {
          ...
          setTimeout(() => {
            ...
            if (el) {
              ...
              setTimeout(() => el.classList.remove('ring-2', 'ring-cyan-500/60'), 2500)
            }
          }, 350)
        } else if (userEmail) {
          // Candidates not in an organization belong on public /pricing
  ...
  }, [isEnterpriseMember, userEmail, userPlan, enterpriseRole])
    ...
    <div className="min-h-screen bg-[#000000] light:bg-white text-zinc-100 light:text-zinc-900 flex flex-col font-sans selection:bg-zinc-800 selection:text-white relative">
              ...
              {isUserMenuOpen && (
                <>
                  ...
                  <div className="absolute left-0 mt-2 w-72 rounded-xl bg-[#0c0c0e] light:bg-white border border-zinc-800 light:border-zinc-200 shadow-2xl py-2 z-50 text-xs animate-in fade-in zoom-in-95 duration-150">
                    ...
                    {/* Refer & Earn — 3rd from bottom above Sign Out */}
                    {userRole !== 'admin' && (
                      <Link
                        href="/profile#referrals"
                        onClick={() => setIsUserMenuOpen(false)}
                        className="flex items-center gap-2.5 px-3.5 py-2 text-emerald-300 light:text-emerald-700 hover:text-white light:hover:text-zinc-900 hover:bg-emerald-950/30 transition-colors"
                      >
                        <Gift className="w-3.5 h-3.5 text-emerald-400 light:text-emerald-600 shrink-0" />
                        <div className="flex-1 min-w-0">
                          <div className="font-medium">Refer &amp; Earn</div>
                          <div className="text-[10px] text-zinc-500 light:text-zinc-600">Get ₹150 per referral to Bank/UPI</div>
                        </div>
                        <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-emerald-950 text-emerald-300 light:text-emerald-700 border border-emerald-800 font-bold">₹150</span>
                      </Link>
                    )}
                      {userRole !== 'admin' && (
                        <button
      ...
      {isMobileNavOpen && (
        <div className="md:hidden">
          ...
          <div className="fixed top-14 left-3 right-3 max-w-sm mr-auto z-50 p-4 bg-zinc-950/95 light:bg-white/95 backdrop-blur-2xl border border-zinc-800/90 light:border-zinc-200 rounded-2xl shadow-2xl shadow-black/80 light:shadow-zinc-300/60 space-y-3.5 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="space-y-1 text-xs">
              ...
              {/* Refer & Earn — above Sign Out */}
              {userRole !== 'admin' && (
                <Link
                  href="/profile#referrals"
                  onClick={() => setIsMobileNavOpen(false)}
                  className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg bg-emerald-950/30 hover:bg-emerald-950/50 border border-emerald-800/40 text-emerald-300 light:text-emerald-700 hover:text-white transition-colors"
                >
                  <Gift className="w-4 h-4 text-emerald-400 light:text-emerald-600 shrink-0" />
                  <span className="font-semibold">Refer &amp; Earn</span>
                  <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-emerald-950 text-emerald-300 border border-emerald-700 ml-auto font-bold">₹150 CASH</span>
                </Link>
              )}
                  <button
